...
class DBConnection:
    inmemorydbconnection = None
    def __init__(self, filename=None):    
        #fall back to the database given in the config
        if not filename:
            filename = headphones.DB_FILE        
        
        self.filename = filename
        if self.filename == ":memory:":
            if not DBConnection.inmemorydbconnection:
                DBConnection.inmemorydbconnection = sqlite3.connect(filename, timeout=20,check_same_thread = False)
            self.connection = DBConnection.inmemorydbconnection
        else:
